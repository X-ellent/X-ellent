...
extern struct addon *find_addon(struct addon *firstaddon,int typ) {
    struct addon *ao;
...
extern int addon_level(struct addon *firstaddon,int typ) {
    struct addon *ao;
    for (ao=firstaddon;ao;ao=ao->next)
...
extern struct addon*  strip_addons(struct player *p,struct addon *a) {
	...
	if (ca->damage>0) {
	    ...
	    if (ca->level>1) {
		sprintf(txt,"Your %s has been damaged.",ca->is->name);
		...
		ca->level=0;
		new_addon_level(p,ca);
		sprintf(txt,"Your %s has been destroyed.",ca->is->name);
		free_addon(ca);
	    }
...
extern int addon_command(struct player *p,struct addon *a,unsigned char *s) {
	...
	if (strcmp(s,"STATUS")==0) {
	    psend(p,"=VIS STATUS\n");
	    psend(p,(sprintf(txt,"=M%d %d,%d S%d\n",a->info[0],a->info[2],
