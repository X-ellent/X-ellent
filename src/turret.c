...
void generate_pass(char *p,int n) {
	for (int i=0;i<n;i++) p[i]=random()%10+'0';
}

void damage_turret(struct turret *t,int d,struct player *p) {
	char txt[80];
	t->shield-=d;
	if (p) p->score+=(d+9)/10;
	if (t->shield<0) {
		...
		if (p) {
			sprintf(txt,"%s has just destroyed a turret",p->name);
			global_message(txt);
			if ((t->flags&TFLG_VINDICTIVE)&&(t->victim==p)) p->cash+=150;
			else p->cash+=55;
			p->score+=100;
			p->tkills++;
			if (p->rating<10 || (t->victim!=p && p->rating<15))
				p->rating++;
			t->victim=p;
		}
	}
...
void update_turrets() {
	int rd, maxspin, xo,yo;
	if (!firstturret) return; /* No turrets! */
	do_active_turret();
	for (struct turret *t=firstturret;t;t=t->next) {
		if (t->frame<0) {
			if (!(++t->frame)) {
				t->flags&=~TFLG_DESTROYED;
				t->shield=TURRET_SHIELD;
			...
			else if (rd>maxspin) { t->rot+=maxspin; t->rot=t->rot%360; }
			else t->rot=t->targ;
			if (t->frame>0) {
				t->frame++;
				if (t->frame>=3) {
