...
static int getnextplayer(struct player *p);
static void get_lift_info(struct login *t,int lift,char *rc);
static int terminal_login(int n,char *in);
...
extern void run_program(struct player *p) {
	...
	case 0:
		...
		n=(int)t->num[--t->nsp];
		s=(n>=ROMBASE)?s=&rom[n-ROMBASE]:&t->ram[n];
		if (t->status&PFLG_HI) {
		...
		case 10:aa=POP;bb=POP;
		    ss=(bb>=ROMBASE)?&rom[bb-ROMBASE]:&t->ram[bb];
		    player_message((struct player *)aa,ss);break;
		case 11:aa=POP;
		    s=(aa>=ROMBASE)?&rom[aa-ROMBASE]:&t->ram[aa];
		    global_message(s);break;
		case 12:ply=(struct player *)POP;aa=POP;
		    ply->cash+=aa;break;
...
extern int terminal_input(int n,int state,char *in) {
	...
	if (tuser[n]->channel!=-1) {
	    send(n,"!That user has a connection already\n");
	    tuser[n]=0;
	    return 0;
	}
	tuser[n]->channel=n;
	...
	if (!(*p->pass)) {
	    send(n,"!No Password Set!\n");
	    terminal_disconnect(n);
	    return 0;
	}
	if (strcmp(p->pass,in)) {
	    send(n,"!Invalid password\n");
	    terminal_disconnect(n);
	    return 0;
	}
